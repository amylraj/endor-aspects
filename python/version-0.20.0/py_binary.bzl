...
internal_endor_py_binary_resolve_dependencies = aspect(
    attr_aspects = ["deps"],
...
def _endor_py_binary_get_callgraph_metadata(target, ctx):
    return [OutputGroupInfo(endor_python_info = depset([]))]

internal_endor_py_binary_generate_callgraph_metadata = aspect(
    attr_aspects = ["deps"],
